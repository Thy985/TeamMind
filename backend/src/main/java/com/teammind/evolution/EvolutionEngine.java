...
public class EvolutionEngine {
    ...
    /**
     * 进化门禁决策服务（非构造器注入，保持测试兼容；为 null 时跳过门禁）
     */
    ...
    public EvolutionResultDTO evolve(Agent agent, EvolutionType type, EvolutionRequest request) {
        ...
        // 进化门禁决策：默认放行；若门禁拒绝，则给出可解释原因并停止
        EvolutionGateService.GateDecision gate = evaluateGate(agent, type, request);
        if (!gate.allowed()) {
            log.info("Evolution gated for agent={}, type={}: {}", agent.getId(), type, gate.reason());
            return EvolutionResultDTO.builder()
                    .agentId(agent.getId())
                    .type(type.name())
                    .fromVersion(agent.getEvolutionVersion())
                    .toVersion(agent.getEvolutionVersion())
    ...
    private EvolutionResultDTO generateTool(Agent agent, EvolutionRequest request) {
        ...
        // 执行能力声明：让 AgentExecutionEngine 能调度该工具（生成即可用）
        if (newTool.get("toolType") == null) {
            newTool.put("toolType", inferToolType(newTool));
        }
    ...
    /**
     * 进化门禁评估：支持通过请求上下文 "force=true" 显式强制跳过门禁。
     * gateService 未注入（测试场景）时默认放行。
     */
    private EvolutionGateService.GateDecision evaluateGate(
            Agent agent, EvolutionType type, EvolutionRequest request) {
        if (gateService == null) {
            return EvolutionGateService.GateDecision.allow("Gate service not configured");
        }
        // 显式强制进化时跳过门禁（人工高权限操作）
        if (request != null && request.getContext() != null
                && Boolean.TRUE.equals(request.getContext().get("force"))) {
            return EvolutionGateService.GateDecision.allow("Forced evolution by request");
        }
        return gateService.evaluate(agent, type);
    }
    ...
    /**
     * 推断工具的执行能力类型（toolType），供 AgentExecutionEngine 调度。
     * 依据工具名/描述关键词映射到内置真实执行能力。
     */
    private String inferToolType(Map<String, Object> tool) {
        String name = String.valueOf(tool.getOrDefault("name", ""));
        String description = String.valueOf(tool.getOrDefault("description", ""));
        String haystack = (name + " " + description).toLowerCase();

        if (haystack.contains("code") || haystack.contains("analy") || haystack.contains("scan")) {
            return "code_analyzer";
        }
        if (haystack.contains("web") || haystack.contains("search") || haystack.contains("query")) {
            return "web_search";
        }
        if (haystack.contains("file") || haystack.contains("read")) {
            return "file_reader";
        }
        // 兜底：文本处理（内置真实能力最通用）
