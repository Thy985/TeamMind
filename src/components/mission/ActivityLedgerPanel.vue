...
import { NCard, NEmpty, NSpin, NTag, NIcon, NText, NSpace, NButton, useMessage } from 'naive-ui'
import {
...
import { taskDetailApi, knowledgeApi } from '@/api/axios'
...
// ─── Knowledge Candidate state (from backend) ──────────────
// Candidates are generated by ActivityExtractor with noise filtering.
// Frontend tracks saved/dismissed state for UI.
const savedIds = ref<Set<string>>(new Set())
const dismissedIds = ref<Set<string>>(new Set())

const activeCandidates = computed(() => {
  const candidates = activity.value?.knowledgeCandidates || []
  return candidates.filter((c: any) => !savedIds.value.has(c.id) && !dismissedIds.value.has(c.id))
})

async function saveCandidate(c: any) {
  try {
...
function dismissCandidate(c: any) {
  dismissedIds.value.add(c.id)
